...
def read_label_files(ANN, pick, exclusive = False):
    ...
    for i, file in enumerate(annotations):
        ...
        print(file)
        all = list()
        lines = [line for line in in_file.readlines() if len(line) > 2]
        jpg = ''
        for line in lines:
            jpg, w, h, name, xn, yn, xx, yx = line.split(' ')
            w, h = int(w), int(h)
            xn, yn = int(xn), int(yn)
            xx, yx = int(xx), int(yx)
            xn, xx = min(xn, xx), max(xn, xx)
            yn, yx = min(yn, yx), max(yn, yx)
            print(f'{name}: ({xn},{yn}), ({xx},{yx})')
...
def parse(self, exclusive = False):
    ...
    dumps = read_label_files(ann, meta['labels'], exclusive)
    return dumps
    ...
    for i in range(self.FLAGS.epoch):
        ...
        for b in range(batch_per_epoch):
            ...
            for j in range(b*batch, b*batch+batch):
                ...
                if len(train_instance[0]) < 4: continue
                try:
